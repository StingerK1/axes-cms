...
$query = "SELECT * FROM discussion ORDER BY datetime DESC LIMIT " . $first_page . ',' . $rows_per_page;
$statement = $db->prepare($query);
$statement->execute();
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="list-group">
                <a href="user_discussion.php" class="list-group-item active main-color-bg"><span class="glyphicon glyphicon-music" aria-hidden="true"></span> Discussions</a>
            ...
            <div class="col-md-9 mt-5">
                ...
                <div class="panel-heading main-color-bg">
                    <h3 class="panel-title">Discussion <span class="badge"><?= $number_of_rows ?></span></h3>
                </div>
                <div class="panel-body">
                    <div class="row">
                    <div class="text-end col-md-12">
                        <a class="btn btn-primary" href="discussion_create.php" role="button">Post in Discussion</a>
                    </div>
                    </div>
                    <br>
                    ...
                    <?php while($row = $statement->fetch()): ?>
                        <tr>
                        <td><?= $row['title'] ?></td>
                        <td><?= strlen($row['discussionPost']) >= 50 ? substr($row['discussionPost'], 0, 50) . "..." : $row['discussionPost'] ?></td>
                        <td><?= date("M j, Y, g:i a", strtotime($row['datetime'])) ?></td>
                        
                        <input type="hidden" name="discussionId" value="<?= $row['discussionId'] ?>" />
                        <td><a class="btn btn-default" href="discussion_edit.php?discussionId=<?= $row['discussionId']?>">Edit</a></td>
                        </tr>
                    <?php endwhile ?>
                    </table>
                </div>
                ...
                <nav aria-label="Page navigation example" style="max-width:100px;margin:auto;" class="text-center">
                        <ul class="pagination justify-content-center">
                            <?php if ($page > 1): ?>
                                <li class="page-item">
                                    <a href="user_discussion.php?page=<?= ($page - 1) ?>" aria-label="Previous" style="color: #0d6efd;" class="page-link">Previous</a>
                                </li>
                            <?php endif ?>
                            <?php for ($i = 1; $i <= $number_of_pages; $i++): ?>
                                <?php if ($page == $i) { $status = "active"; } else { $status = ""; } ?>
                                <li class="<?= $status ?>">
                                    <a href="user_discussion.php?page=<?= $i ?>" class="page-link" style="background-color: #0d6efd; color: #fff;"><?= $i ?></a>
                                </li>
                            <?php endfor ?>
                            <?php if ($number_of_pages > $page): ?>
                                <li>
                                    <a href="user_discussion.php?page=<?= ($page + 1) ?>" aria-label="Next" class="page-link" style="color: #0d6efd;">Next</a>
                                </li>
                            <?php endif ?>
                        </ul>
                    </nav>
